    ...
    methods: {
        ...
        locatePlayer: function () {
            for (i = 0; i < app.view.gamePlayers.length; i++) {
                ...
                else app.opponent = app.view.gamePlayers[i].player;
            }
        }, //SALVOS
            ...
            for (i = 0; i < app.view.salvoes.length; i++) {
                for (j = 0; j < app.view.salvoes[i].locations.length; j++) {
                    if (app.mainPlayer.id == app.view.salvoes[i].playerId) {
                        var elements2 = document.getElementById(
                            app.view.salvoes[i].locations[j]
                        );
        ...
        salvoAim: function (salvo) {
            if (app.salvoes.locations.length < 5) {
                ...
                app.salvoes.locations.length == 5 ||
                app.salvoes.locations.some((loc) => loc == salvo)
            ) {
                document.getElementById(salvo).classList.remove("aim");
                app.salvoes.locations = app.salvoes.locations.filter(
                    (salvoes) => salvoes != salvo
                );
            } else {
                alert("Para de disparar, rambo");
            }
        },
        salvoFire: function () {
            var mainPlayerSalvoes = app.view.salvoes.filter(
                (salvoes) => salvoes.playerId == app.mainPlayer.id
            );
            var opponentSalvoes = app.view.salvoes.filter(
                (salvoes) => salvoes.playerId != app.mainPlayer.id
            );
            ...
            $(".grid-stack-item").each(function () {
                ...
                if ($(this).attr("data-gs-width") !== "1") {
                    for (var i = 0; i < parseInt($(this).attr("data-gs-width")); i++) {
                        coordinate.push(
                            String.fromCharCode(parseInt($(this).attr("data-gs-y")) + 65) +
                            (parseInt($(this).attr("data-gs-x")) + i + 1).toString()
                        );
                    }
                } else {
                    for (var i = 0; i < parseInt($(this).attr("data-gs-height")); i++) {
                        coordinate.push(
                            String.fromCharCode(
                                parseInt($(this).attr("data-gs-y")) + i + 65
                            ) + (parseInt($(this).attr("data-gs-x")) + 1).toString()
                        );
                    }
                }
        ...
        gridCreation: function () {
            ...
            if (app.view.ships.length == 0) {
                ...
                grid.addWidget(
                    ...
                    grid.addWidget(
                        '<div><div id="PatrolBoat" class="grid-stack-item-content PatrolBoatHorizontal"></div><div/>', {
                            width: 2,
                            heigth: 1,
                            x: 0,
                            y: 0,
                ...
                $("#Carrier,#Battleship,#Submarine,#Destroyer,#PatrolBoat").click(
                    function () {
                        ...
                        if (
                            $(this)
                            .children()
                            .hasClass(idShip + "Horizontal") &&
                            y <= yShip &&
                            grid.isAreaEmpty(x, y + 1, heigthShip, widthShip)
                        ) {
                            console.log(yShip);
                            ...
                            $(this)
                                .children()
                                .removeClass(idShip + "Horizontal");
                            $(this)
                                .children()
                                .addClass(idShip + "Vertical");
                        } else if (
                            $(this)
                            .children()
                            .hasClass(idShip + "Vertical") &&
                            x <= xShip &&
                            grid.isAreaEmpty(x + 1, y, heigthShip, widthShip)
                        ) {
                            console.log(xShip);
                            ...
                            $(this)
                                .children()
                                .addClass(idShip + "Horizontal");
                            $(this)
                                .children()
                                .removeClass(idShip + "Vertical");
                        }
                    }
                );
                ...
                ships.forEach((ship) => {
                    ...
                    ship.parentElement.onclick = function (event) {
                        ...
                        let itemHeight = parseInt(
                            itemContent.parentElement.dataset.gsHeight
                        );
                        ...
                        if (itemContent.classList.contains(itemContent.id + "Horizontal")) {
                            ...
                            if (
                                grid.isAreaEmpty(itemX, itemY + 1, itemHeight, itemWidth - 1) &&
                                itemY + (itemWidth - 1) <= 9
                            ) {
                            ...
                            if (
                                grid.isAreaEmpty(itemX + 1, itemY, itemHeight - 1, itemWidth) &&
                ...
                for (var i = 0; i < app.view.ships.length; i++) {
                    ...
                    let yShip =
                        parseInt(ship.locations[0].slice(0, 1).charCodeAt(0)) - 65;
        ...
        gameHistory: function () {
            var mainSalvoes = this.view.salvoes.filter(
                (salvoes) => salvoes.playerId == app.mainPlayer.id
            );
            var gameHistory = [];
        ...
        opponentGameHistory: function () {
            var oppSalvoes = app.view.salvoes.filter(
                (salvoes) => salvoes.playerId != app.mainPlayer.id
            );
            var oppGameHistory = [];
        ...
        checkGameState: function () {
            if (
                app.view.gameState == "WAITING_OPPONENT" ||
                app.view.gameState == "WAITING_OPPONENT_SHIPS" ||
                app.view.gameState == "OPPONENT'S_ATTACKING"
            ) {
